...
#alphabet pruning help function for player's move
def get_max(grid,player,depth,a,b):
    ...
    for i in range(0, N):
        for j in range(0, N):
            if grid[i][j] == 0:
                ...
                score = get_min(temp_grid,3-player,depth-1,a,b)
                score[0] = i
                score[1] = j
...
#alphabet pruning help function for ai's move
def get_min(grid,player,depth,a,b):
    if depth==0 or terminate_condition(grid):
        player_score = player_win_state(grid)
        return [-1, -1, player_score]
    min_score = [-1,-1,b]
    for i in range(0, N):
        for j in range(0, N):
            if grid[i][j] == 0:
                temp_grid = copy.deepcopy(grid)
                update_board(temp_grid,i, j,player)
                score = get_max(temp_grid,3-player,depth-1,a,b)
                score[0]=i
                score[1]=j
...
#alphabet pruning
def alphabet_pruning(grid,player,depth,a,b):
    if player==1:
        return get_max(grid,player,depth,a,b)
    else:
        return get_min(grid,player,depth,a,b)
...
s= alphabet_pruning(Grid,1,5,float('-inf'),float('inf'))
end = time.time()
print s
print end-start
f = open("output.txt", "w")
